...
TRIAL_QUOTA = 1 * 1024**3
BASIC_QUOTA = 2 * 1024**3
PRO_QUOTA = 5 * 1024**3
...
@app.get("/api/v1/me/files")
def my_files(user=Depends(current_user)):
    rows = query(
        "SELECT id, size, created_at FROM parts WHERE user_id=? ORDER BY created_at",
        (user["id"],),
        fetch=True,
    )
    return {
        "email": user["email"],
        "plan": user["plan"],
        "plan_expires_at": user["plan_expires_at"] if "plan_expires_at" in user.keys() else None,
        "quota_bytes": user["quota_bytes"],
        "used_bytes": used_bytes(user["id"]),
        "parts": [
            {"id": r["id"], "size": r["size"], "created_at": r["created_at"]}
            for r in rows
        ],
    }
...
async def upload_part(request: Request, user=Depends(current_user)):
    ...
    plan = user["plan"]
    max_bytes = MAX_SINGLE_BYTES if plan in ("trial", "basic") else MAX_PART_BYTES
